...
export function initState (vm: Component) {
  ...
  // Vue.js对options中的数据进行"响应式"(即双向绑定)的过程
  vm._watchers = []
...
function initData (vm: Component) {
  ...
  while (i--) {
    ...
    if (props && hasOwn(props, key)) {
      ...
      proxy(vm, `_data`, key)     // 将data上的属性挂在到vm上
    }
  }
  // observe data
  // observe会通过defineReactive对data中的对象进行双向绑定
  // 最终通过Object.defineProperty对对象设置setter和getter的方法
  // getter方法主要用来进行依赖收集
  // setter方法会在对象被修改的时候触发(不包含添加属性，添加属性用Vue.set)，这时setter会通知闭包中的Dep
...
// computed watcher就有lazy(懒计算)特性
// 比如有个computed是b，b依赖a，当a改变的时候，b并不会立即重新计算，只有其他地方需要读取b时，才会真正计算
...
function initComputed (vm: Component, computed: Object) {
  ...
  for (const key in computed) {
    ...
    if (!(key in vm)) {
      // 若computed中的key没有设置到vm中，通过defineComputed函数挂载上去
      defineComputed(vm, key, userDef)
    } else if (process.env.NODE_ENV !== 'production') {
      // 若data和props有和computed中的key重名的，会产生warning
      if (key in vm.$data) {
...
function createComputedGetter (key) {
  return function computedGetter () {
    ...
    if (watcher) {    // watcher存在时，判断watcher.dirty是否存在
      // 第一次新建watcher实例时以及data变化需要更新时，this.dirty为true，则调用watcher.evaluate，否则缓存取之前的value
      if (watcher.dirty) {
        watcher.evaluate()
      }
      if (Dep.target) {
...
function initWatch (vm: Component, watch: Object) {
  ...
  for (const key in watch) {
    // 将每个watch[key]赋值给handler
    const handler = watch[key]
    if (Array.isArray(handler)) {   // 若是数组，则遍历
...
function createWatcher (
  ...
  return vm.$watch(expOrFn, handler, options)      // 调用了Vue.prototype.$watch方法
}
...
export function stateMixin (Vue: Class<Component>) {
  ...
  Vue.prototype.$watch = function (
    ...
    const watcher = new Watcher(vm, expOrFn, cb, options)   // 实例化一个watcher，这是个user watcher
